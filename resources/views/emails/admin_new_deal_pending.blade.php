...
<table border="0" cellpadding="0" cellspacing="0" width="60%" style="margin:0 auto;border:1px solid rgba(0, 0, 0, 0.2);padding:20px;">
 ...
 <tr>
  <td style="padding: 20px 0 30px 0;line-height:22px;">
    <h2>New Deal Pending Approval</h2>
    <p>A new deal has been submitted and requires your review.</p>
    <p><strong>Deal Details:</strong></p>
    <ul>
        <li>Title: <?php echo $deal->title ?></li>
        <li>Vendor: <?php echo $deal->vendor->first_name ?> <?php echo $deal->vendor->last_name ?> (<?php echo $deal->vendor->email ?>)</li>
        <li>Price: $<?php echo number_format($deal->deal_price, 2) ?></li>
        <li>Inventory: <?php echo $deal->inventory_total ?> spots</li>
        <li>Submitted: <?php echo $deal->created_at->format('F d, Y g:i A') ?></li>
    </ul>
    <p><a href="{{ url('/admin/deals?tab=pending') }}" style="background: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block; margin-top: 10px;">Review Deal</a></p>
  </td>
 </tr>
